...
class Gifts(db.Model): 
    ...
    #dguests =db.relationship("Guest_gift", back_populates="gift_deets")
    
class Comment(db.Model): 
    comment_id = db.Column(db.Integer(), primary_key=True, autoincrement=True)
    comment_guest =db.Column(db.Integer(), db.ForeignKey('guests.guest_id'))
    comment_content =db.Column(db.Text(), nullable=True)
    comment_date =db.Column(db.DateTime(), default=datetime.utcnow())

    commenter = db.relationship("Guests", backref="guest_comments")
...
class Lga(db.Model): 
    ...
    lga_name =db.Column(db.String(100), nullable=True)
...
class State(db.Model): 
    ...
    state_slogan =db.Column(db.String(100), nullable=False)  
